...
```text
❯ 输入消息
╭ 󰉋 ~/programming/ai/pi-extensions · ◈ 修复状态栏 ·  main ──────── 󰃭 2026-08-21 16:32 ╮
...
- 多行状态栏使用与终端等宽的圆角框体，首尾状态分别嵌入上下边框
- 第一行右端显示本地日期和时间，精确到分钟；空间不足时自动隐藏
...
- 上下文与 session token/cache 统计独占第二行；已知的 `mcp` 和 `pi-lens-lsp` 状态显示在第三行，顺序为 `󰒍 MCP:`、`LSP Active`，并保留原状态文本和主题颜色
- 未知或改名后的第三方扩展状态不会丢失，会动态保留在主状态行；已知 key 只用于特殊布局和 MCP 图标
