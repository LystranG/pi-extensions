...
- 第二行显示累计 session token：输入使用 `↓`，输出使用 `↑`，缓存创建使用 `W`，缓存读取使用 `R`，最近一次请求的缓存命中率使用 `CH`；有模型 token usage 时即使缓存数据为零也显示 `W0K R0K CH0.0%`，单位为 `K`，达到四位数时进位为 `M`
- session token 与缓存统计独占第二行；`pi-mcp-adapter` 的 `mcp` 状态和 pi-lens 的 `pi-lens-lsp` 状态显示在第三行，顺序为 `󰒍 MCP:`、`LSP Active`，并保留原状态文本和主题颜色
