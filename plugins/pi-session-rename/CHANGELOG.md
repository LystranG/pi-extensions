...
- 21e4d4d: Send opencode's per-conversation routing headers (`x-opencode-session`, `x-opencode-client`) on the background title request, so sessions using the `opencode` / `opencode-go` providers are named instead of failing with `400 MissingSessionID` (Thanks @LiusCraft)
...
- 21e4d4d: Give the background title request enough output budget for models whose thinking cannot be turned off, and retry a reply that carries no title before warning instead of leaving the session unnamed without feedback (Thanks @LiusCraft)
