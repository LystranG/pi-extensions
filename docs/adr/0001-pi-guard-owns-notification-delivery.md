...
`@lystran/pi-guard` 在危险命令确认框弹出前自己投递通知：先尝试终端级 OSC 序列，再走 spawn 系统通知进程的降级链，最后兜底写一个 BEL。之所以不把「危险命令待确认」发布到 `pi.events` 交给第三方通知插件消费、也不依赖宿主已有的 `ui_prompt_start` 事件，是因为这是一个安全插件：**装了就有效** 优先于职责分离。生态证据也不支持外包——唯一订阅 `rpiv:ask-user:prompt` 的通知插件默认关闭，`rpiv:ask-user:blocked` 至今零消费者；而 Pi 上游明确拒绝把通知能力做进核心（PR #462 被拒、issue #5788 标 `NOT_PLANNED`，只合并了 OSC 777 示例扩展）。

> 实现状态：已实现（`plugins/pi-guard/src/notify.ts` 的终端 OSC 前置层与 BEL 的 `isTTY` 门控，测试与 README、changeset 同步）。
...
4. **`TMUX` 存在时不要试图用 DCS passthrough 绕过。** 它要求用户配 `allow-passthrough`（默认 off），而 `on` 对**不可见 pane 不生效**——恰好就是「用户切走了、需要通知」这个场景；要覆盖得配 `all`。另外 tmux 的 `update-environment` 不含任何终端身份变量，reattach 后检测会指向旧终端。
...
- **覆盖率确实变窄**：支持 OSC 通知的只有 kitty / Ghostty / WezTerm / iTerm2 / rxvt-unicode / Warp；VS Code 集成终端（xterm.js 官方序列表里没有 9/99/777）、Windows Terminal（777 默认关闭）、Apple Terminal、Alacritty，以及所有 tmux/screen 用户都退回 spawn 链。这不是功能回归，但预期要与用户说明。
